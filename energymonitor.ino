...
const float w_per_pulse = 1;   //My meter flashes 1000 times per kWh
...
void setup() {
  ...
  attachInterrupt(2, chan1_isr, FALLING); //STOFFE kolla upp vilken pinns
  Serial.begin(9600);
  delay(1000);
...
void loop() {
  ...
  static unsigned int chan1_kwh;
  static unsigned int chan2_kwh;
  ...
  if (millis() - report_time > 5000) {
    ...
    if (!(report_time == 0 && chan1_watts == 0 && chan2_watts == 0)) 
    {
      ...
      snprintf(topic, 35, "house/power/meter/all/kwh");
      snprintf(value, 6, "%d", chan2_watts);
      client.publish(topic, value);

      snprintf(topic, 35, "house/power/meter/heat/kwh");
      snprintf(value, 6, "%d", chan2_watts);
      client.publish(topic, value);
